...
test('selects a question, runs and grades a correct answer, records progress', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.getByTestId('progress')).toHaveAttribute('aria-label', /Solved 0 of 54/);
  ...
  await expect(page.getByTestId('progress')).toHaveAttribute('aria-label', /Solved 1 of 54/);
